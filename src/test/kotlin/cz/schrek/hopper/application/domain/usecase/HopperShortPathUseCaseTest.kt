...
class HopperShortPathUseCaseTest {
    ...
    fun `start is on end - should have zero hops`(): Unit = runTest {
        ...
        val result = useCase.searchShortestPath(gameBoardLayout)

        result shouldContainExactly mapOf(
            gameBoardLayout to SearchHooperShortestPathResult.Success(
                listOf(
                    GameTurnSnapshot(
                        order = 0,
    ...
    fun `layout reachable with single hop`() = runTest {
        ...
        result[gameBoardLayout].shouldBeInstanceOf<SearchHooperShortestPathResult.Success>().should {
    ...
    fun `layout reachable with four hops`() = runTest {
        ...
        result[gameBoardLayout].shouldBeInstanceOf<SearchHooperShortestPathResult.Success>().should { result ->
            result.hops
    ...
    fun `path ends in obstacles`() = runTest {
        ...
        result[gameBoardLayout].shouldBeInstanceOf<SearchHooperShortestPathResult.NotFound>().should { result ->
